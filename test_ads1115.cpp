 ...
 * 
 * Expected behavior:
 * - Should show values from 0-26700 range (16-bit ADC)
...
extern bool adsInitialized;

void setup() {
    Serial.begin(115200);
    while (!Serial) delay(10); // Wait for serial port to open
    
    Serial.println("=== ADS1115 Quick Test ===");
    Serial.println("Initializing ADS1115...");
    
    if (!ads.begin()) {
        Serial.println("❌ ERROR: Failed to initialize ADS1115!");
        Serial.println("Check your wiring:");
        Serial.println("  SDA -> Pin 2 (Leonardo/Pro Micro)");
        Serial.println("  SCL -> Pin 3 (Leonardo/Pro Micro)");
